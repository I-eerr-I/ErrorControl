...
class GF:
    ...
    def subs_value(self, poly, val_power):
        to_add = []
        for i in range(len(poly)):
            if i == 0 and poly[i] == 1:
                to_add.append(self(0))
            if poly[i] != 0 and i != 0:
                power = self.pow_ind(val_power, i)
                to_add.append(self.field[power] * poly[i])
...
class DPoly:
    def __init__(self, poly, mod = None):
        if mod:
            self.poly = [poly[i] for i in range(mod)]
            for i in range(len(poly)):
                if i >= mod:
                    j = i%mod
                    self.poly[j] = poly[i] + self.poly[j]
        else:
            self.poly = poly
